...
# pyautogui aborts -- by raising FailSafeException out of whatever call is in flight --
# whenever the cursor is sitting in a screen corner. That check runs before every call
# and again on every step of a tweened move, and it reads the cursor's *current*
# position, so it does not care who put it there. Anyone using the machine while the bot
# runs trips it: the Start button, a window's close box, a fling into a corner to reach
# the desktop. The bot then dies mid-career with a traceback on stderr and nothing in the
# log, which is the "it sometimes stops" this turns off.
#
# The corner is pyautogui's panic switch for scripts that have no other way to be
# stopped. This one does: the hotkey listener thread is independent of the bot thread and
# is not blocked by anything here, and the web UI can stop it too. Turning the corner off
# costs an abort route that is already covered twice over, and buys back every run that
# ended because the mouse passed through the wrong pixel.
#
# Set at import rather than at a call site: FAILSAFE is a module global on pyautogui, so
# one assignment covers every caller in the process, including main.py's window focusing.
...
# pyautogui moves the cursor in a straight line at constant speed, which no hand does.
# Easing out -- quick away from where it was, settling as it arrives -- costs one
# argument and looks more like a real movement than the jitter does.
CURSOR_TWEEN = pyautogui.easeOutQuad
...
def swipe(start_x_y : tuple[int, int], end_x_y : tuple[int, int], duration=0.3):
  ...
  hold()
  # Released in a finally: the button is physically down between these two lines, and
  # anything that escapes the move -- the bot being stopped, a display error -- would
  # otherwise leave it down for the user to find, with every later click read as a drag.
  try:
...
def scroll(clicks, position=None, settle=0.02):
  """Mouse wheel scroll, positive scrolls up. Never presses a button.

  Scrolling a list by swiping means holding the button down over the list, and the game
  reads the mouse-up as a tap on whatever is underneath -- which selects it. The wheel
  has no such failure mode, and has no fling momentum to overshoot with.

  The notches are sent one event at a time rather than as a single batched delta. The
  game normalises a wheel event to one step regardless of how much delta it carries, so
  `pyautogui.scroll(10)` moves exactly as far as `pyautogui.scroll(1)` -- measured at
  about 75px either way. Sending them separately is what actually makes the count mean
  something.
  """
