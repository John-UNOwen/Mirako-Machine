...
export function useImportConfig({
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Fetched again rather than taken as saved: creating the preset switched the page to
      // it, and its first fetch may well have landed before this save -- the new preset's
      // seed, not the import. A load is what rebuilds the page, so this is what shows the
      // import; it used to show the seed until the window next took focus.
      reloadActiveConfig();
